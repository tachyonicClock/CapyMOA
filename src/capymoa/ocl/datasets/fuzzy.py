...
def _idx_interleave(
    ...
    Returns two tensors one for the newly fuzzy left task and one for the newly fuzzy
...
def _fuzzy_task_idx(
    ...
    for i in range(n_tasks - 1):
        ...
        fuzzy_left_idx, fuzzy_right_idx = _idx_interleave(left_idx, right_idx, mask)
        task_idx[i] = fuzzy_left_idx
        task_idx[i + 1] = fuzzy_right_idx
    return task_idx


def fuzzy_sigmoid_transitions(
    tasks: Sequence[Dataset], width: float = 0.5, seed: int | None = None
) -> Sequence[Dataset]:
    r"""Apply sigmoid-based fuzzy task transitions to a sequence of tasks.

    This is a convenience wrapper around :func:`fuzzy_task_transitions` that uses
    :class:`SigmoidFn` as the transition function.
    """
    return fuzzy_task_transitions(tasks, SigmoidFn(width=width, seed=seed))


def fuzzy_task_transitions(
    tasks: Sequence[Dataset], transition_fn: TransitionFn = SigmoidFn(width=0.5)
) -> Sequence[Dataset]:
    r"""Apply fuzzy task transitions to a sequence of tasks using the provided transition
    function.

    We define a combined stream with a fuzzy boundary :math:`S` of length :math:`n`
    given two source streams :math:`L` and :math:`R` of lengths :math:`n_L` and
    :math:`n_R`.
    Our fuzzy boundary preserves the relative order of elements within their
    ...
    return [Subset(stream, idx) for idx in _fuzzy_task_idx(task_idx, transition_fn)]  # type: ignore
