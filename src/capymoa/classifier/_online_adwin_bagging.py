...
class OnlineAdwinBagging(MOAClassifier):
    ...
    def __init__(
        ...
        if (number_of_jobs is None or number_of_jobs == 0 or number_of_jobs == 1) and (
            ...
            moa_learner = "moa.classifiers.meta.OzaBagAdwin"
            ...
            moa_learner = "moa.classifiers.meta.minibatch.OzaBagAdwinMB"
            CLI = f"-l {self.base_learner} -s {self.ensemble_size} -c {self.number_of_jobs} -b {self.minibatch_size} "

        super().__init__(
            CLI=CLI, random_seed=random_seed, java_learner_class=moa_learner
        )
